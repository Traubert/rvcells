...
- **Chain processes.** `Chain(wealth * return, 1000)` defines an iterative Monte Carlo process in a single cell. Referenced distributions are auto-resampled each step. Access any step with bracket syntax (`wealth[12]`), search for a milestone with `ChainIndex(wealth, mean(wealth) > 1000000)`, or view the full trajectory as a fan chart in the Timeline tab.
